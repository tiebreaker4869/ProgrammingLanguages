...
let consume_string_literal cs = 
  ...
  let rec str_matcher chars = 
    match chars with
    | [] -> lexical_error "quote do not match"
    | '\"' :: tl -> ("", tl)
    | hd :: tl -> let (tail_str, remainder) = str_matcher tl in ((char_to_string hd) ^ tail_str, remainder)
  in
  match cs with
    '\"' :: cs -> str_matcher cs
...
let consume_keyword cs = 
  ...
  let rec alphabetical_matcher chars = 
    match chars with
    | [] -> ("", [])
    | hd :: tl -> if not (is_alpha hd) then ("", hd :: tl)
    else let (tail_str, remainder) = alphabetical_matcher tl in
    (char_to_string hd ^ tail_str, remainder)
  in let (alphas, remainder) = alphabetical_matcher cs in
  let lookup = assoc (alphas, lookup_table) in
  match lookup with
  | None ->  lexical_error "Not start with keywords"
  | Some keyword -> (keyword, remainder)
...
let tokenize_char_list cs = 
  let rec go (cs,acc) =
    ...
    | ' ' :: cs -> go (cs, acc) (* ignore blankspaces*)
    | '}' :: cs -> go (cs, (RBrace :: acc))
    ...
    | c :: cs ->
       ...
       then
         ...
         go (cs, (NumLit s :: acc))
               ...
               as below. *)
       if c = '\"' 
       then let (s, cs) = consume_string_literal (c :: cs) in go (cs, StringLit s :: acc)
       else 
       if is_alpha c
       then let (keyword, cs) = consume_keyword (c :: cs) in go (cs, keyword :: acc)
      else   
...
let tokenize (s : string) : token list =
  (* TODO, 1 line *) let cs = char_list_of_string s in tokenize_char_list cs
...
let parse_string (ts : token list) : string * token list =
  ...
  match ts with
  | (StringLit s) :: tl -> (s, tl)
  | _ -> syntax_error (ts, "No string literals at beginning")
...
let expect (t, ts) = 
  (* TODO, about 6 lines *)
  match ts with
  | [] -> syntax_error (ts, "no more tokens there.")
  | hd :: tl -> if hd = t then tl else syntax_error (ts, ("no expected " ^ (string_of_token hd) ^ " token here."))
...
let rec parse_json (ts : token list) : json * token list =
  ...
  let parse_field_value (ts : token list) : (string * json) * token list =
    (* TODO, about 7 lines *)
    match ts with
    | f :: c :: j :: tl -> let (field_name, ts) = parse_string ts in
    let ts = expect (Colon, ts) in
    let (json_value, ts) = parse_json ts
    in ((field_name, json_value), ts)    
    | _ -> syntax_error (ts, "not field value here.") 
  in
  ...
  let rec parse_field_value_list (ts : token list) : (string * json) list * token list =
    (* TODO, about 15 lines *)
    match ts with
    | [] -> syntax_error (ts, "no matched braces")
    | RBrace :: tl -> ([], tl)
    | hd :: tl -> let ((f, j), ts) = parse_field_value ts in
      match ts with
      | Comma :: tl -> let (fvlst, ts) = parse_field_value_list tl in ((f, j) :: fvlst, ts)
      | _ :: tl -> ([(f, j)], tl) 
      | _ -> syntax_error (ts, "should be a closing brace here")
  in
  ...
  let rec parse_array_element_list (ts : token list) : json list * token list =
    (* TODO, about 15 lines *)
    match ts with
    | [] -> syntax_error (ts, "no matched bracket")
    | RBracket :: tl -> ([], tl)
    | hd :: tl -> let (j, ts) = parse_json ts in
      match ts with
      | Comma :: tl -> let (jlst, ts) = parse_array_element_list tl in (j :: jlst, ts)
      | _ :: tl -> ([j], tl)
      | _ -> syntax_error (ts, "should be a closing bracket here")
  in
  ...
  | NullTok :: ts -> (Null, ts)
  | LBracket :: ts -> let (jlst, ts) = parse_array_element_list ts in (Array jlst, ts)
  | LBrace :: ts -> let (fvlst, ts) = parse_field_value_list ts in (Object fvlst, ts)
